...
def plot_chain(chain, joints, ax, name="chain"):
    ...
    # Plot the chain
    lines = ax.plot([x[0] for x in nodes], [x[1] for x in nodes], [x[2] for x in nodes], color='blue', linewidth=3, label=name)
    # Plot of the nodes of the chain
    # Note: Be sure that the nodes have the same color as the chain
    ax.scatter([x[0] for x in nodes], [x[1] for x in nodes], [x[2] for x in nodes], color='blue', s=5)
