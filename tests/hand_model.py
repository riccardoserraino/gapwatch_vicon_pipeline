...
middle_chain = Chain(name='middle', links=[
    URDFLink(
        name="joint1",
        translation_vector=[0.09, 0.03, -0.025], # origin knucle
        orientation=[0, 0, 0], 
...
for i in range(min_length):
    ...
    angle_little = get_closure_angle(point_h3, nodes_little, np.array([x_h3[0] + 0.1, y_h3[0], z_h3[0]]))
    angle_ring = get_closure_angle(point_h4, nodes_ring, np.array([0.16272325, 0.00817894, z_h4[0]]))
