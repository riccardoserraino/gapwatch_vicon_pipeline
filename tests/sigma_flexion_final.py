...
test_dataset = "pen1.bag"      # <-- Change here to use a different file
...
for i in range(min_length):
    ...
    angle_little = get_closure_angle_plot(point_h3, nodes_little, np.array([x_h3[0] + 0.1, y_h3[0], point_h3[2]]))
    angle_ring = get_closure_angle_plot(point_h4, nodes_ring, np.array([0.16272325, 0.00817894, point_h4[0]]))
    angle_middle = get_closure_angle_plot(point_h5, nodes_middle, np.array([0.17139323, 0.06217872, point_h5[0]]))
    angle_index = get_closure_angle_plot(point_h6, nodes_index, np.array([0.12973476, 0.09361607, point_h6[0]]))
    angle_thumb = get_closure_angle_plot(point_h1, nodes_thumb, np.array([-0.03948546, 0.08916224, point_h1[0]]))
